...
$pdf->Cell(25.5,0.7,"Laporan Kondisi Jaringan Loket",0,10,'C');
$pdf->ln(1);
$pdf->SetFont('Arial','B',10);
...
$pdf->Cell(4, 0.8, 'Nama Loket', 1, 0, 'C');
...
$query=mysqli_query($koneksi,"SELECT * from kondisi_jaringan_loket INNER JOIN jaringan_loket ON jaringan_loket.kode = kondisi_jaringan_loket.kode_jaringan where `nama_stasiun`='$nama_stasiun' and tanggal between '$awal' and '$akhir'");
while($lihat=mysqli_fetch_array($query)){
	$pdf->Cell(1, 0.8, $no , 1, 0, 'C');
	$pdf->Cell(3, 0.8, $lihat['kode_jaringan'],1, 0, 'C');
	$pdf->Cell(4, 0.8, $lihat['nama_loket'],1, 0, 'C');
...
$pdf->Output("laporan_jaringan_loket.pdf","I");
