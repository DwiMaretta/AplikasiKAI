...
$pdf->Cell(25.5,0.7,"Laporan Kondisi Jaringan Tempat Duduk",0,10,'C');
$pdf->ln(1);
$pdf->SetFont('Arial','B',10);
...
$pdf->Cell(4, 0.8, 'Nama Tempat Duduk', 1, 0, 'C');
...
$query=mysqli_query($koneksi,"SELECT * from kondisi_jaringan_td INNER JOIN jaringan_td ON jaringan_td.kode = kondisi_jaringan_td.kode_jaringan where `nama_stasiun`='$nama_stasiun' and tanggal between '$awal' and '$akhir'");
while($lihat=mysqli_fetch_array($query)){
	$pdf->Cell(1, 0.8, $no , 1, 0, 'C');
	$pdf->Cell(3, 0.8, $lihat['kode_jaringan'],1, 0, 'C');
	$pdf->Cell(4, 0.8, $lihat['nama_td'],1, 0, 'C');
...
$pdf->Output("laporan_jaringan_td.pdf","I");
