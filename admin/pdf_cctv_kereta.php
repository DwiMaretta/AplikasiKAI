...
$pdf->Cell(25.5,0.7,"Laporan Kondisi CCTV Kereta",0,10,'C');
$pdf->ln(1);
$pdf->SetFont('Arial','B',10);
...
$pdf->Cell(3, 0.8, 'Nama Kereta', 1, 0, 'C');
$pdf->Cell(9, 0.8, 'Kelas', 1, 0, 'C');
...
$awal=$_GET['awal'];
$akhir=$_GET['akhir'];
$nama_kereta=$_GET['nama'];
$query=mysqli_query($koneksi,"SELECT * from kondisi_cctv_di_kereta INNER JOIN cctv_kereta ON cctv_kereta.kode = kondisi_cctv_di_kereta.kode_cctv where `nama_kereta`='$nama_kereta' and tanggal_cek between '$awal' and '$akhir'");

while($lihat=mysqli_fetch_array($query)){
	$pdf->Cell(1, 0.8, $no , 1, 0, 'C');
	$pdf->Cell(3, 0.8, $lihat['kode_cctv'],1, 0, 'C');
	$pdf->Cell(3, 0.8, $lihat['nama_kereta_cek'],1, 0, 'C');
	$pdf->Cell(9, 0.8, $lihat['lokasi'], 1, 0,'C');
	$pdf->Cell(3, 0.8, $lihat['tanggal_cek'],1, 0, 'C');
...
$pdf->Output("laporan_cctv_kereta.pdf","I");
